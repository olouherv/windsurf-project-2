                ...
                <div>
                    ...
                    <select name="student_id" class="mt-1 w-full border-gray-300 rounded-md" required>
                        @foreach($students as $s)
                            <option value="{{ $s->id }}" {{ old('student_id', $internship->student_id) == $s->id ? 'selected' : '' }}>{{ $s->full_name }} ({{ $s->student_id }})</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div>
                        ...
                        <select name="supervisor_teacher_id" class="mt-1 w-full border-gray-300 rounded-md">
                            ...
                            @foreach($teachers as $t)
                                <option value="{{ $t->id }}" {{ old('supervisor_teacher_id', $internship->supervisor_teacher_id) == $t->id ? 'selected' : '' }}>{{ $t->full_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
